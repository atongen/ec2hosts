...
dist: test distclean
	for arch in ${ARCH}; do \
		for os in ${OS}; do \
			env GOOS=$${os} GOARCH=$${arch} go build -v ${LDFLAGS} -o dist/ec2hosts-${VERSION}-$${os}-$${arch}; \
		done; \
	done
...
package: sign
	for arch in ${ARCH}; do \
		for os in ${OS}; do \
			tar czf dist/ec2hosts-${VERSION}-$${os}-$${arch}.tar.gz -C dist ec2hosts-${VERSION}-$${os}-$${arch} ec2hosts-${VERSION}-$${os}-$${arch}.asc; \
		done; \
	done
